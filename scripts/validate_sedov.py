...
def validate_sedov_interactive(snapshot_dir):
    ...
    print("Pre-computing global time-series diagnostics...")
    times = []
    e_errors = []
    switch_energies = []
    floor_energies = []
    ...
    def update(val):
        ...
        with h5py.File(target_file, 'r') as f:
            ...
            target_N = cfg.get('mfm_target_neighbors', 64)
            line_target_nenc.set_ydata([target_N, target_N])
        ...
        if len(r_ex) > 1 and r_ex[-1] > 0:
            ...
            u_shock_peak = np.nanmax(u_plot)
            ax_u.set_ylim(-0.1, u_shock_peak)
        else:
